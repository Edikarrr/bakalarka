...
co to je
	digitalni data negeneruji pouze pocitace a internet, ale v dnesni dobe uz temer vsechna digitalni zarizeni. chytre telefony, tablety, domy, auta, pracky. v dnesni dobe se rozviji se tzv internet veci
	
	bezna digitalni data:
	obraz disku zarizeni, obraze pameti zarizeni, informace o sitovem provozu (NetFlow data), sitovy provoz (kompletni komunikace), informace od provozovatele sluzby(cast logu)
	
jak to vznika
	generuji se pri praci s temato zarizenimi. programy zanechavaji v pameti stopy pri behu ( promenne, kod, vstupne vystupni data), uzivatel uklada ruzne druhy dat (ulozeni dokumentu, odeslani emailu). data zaznamenavajici interakci s okolim. tato data zustavaji uvnitr zarizeni a jsou v dusledku zdrojem dukazu.
	
	priklad - odeslani emailu: stopu v pocitaci: prohlizec, webova stranka, je rozpoznatelna pritomnost  prilohy, email putujici siti a log u provozovatele sluzby. S dostatecnyn opravnenim (napr policie) je mozne vyzadat od poskytovatele sluzby zaznam o dane aktivite.
	
skupiny dat
	volatile - existuji pouze po dobu chodu zarizeni
	active - uzivatelsky dostupne - soubory 
	metadata - informace o datech - pouzity format, cas vytvoreni... priklad fotografie - cas zarizeni, kde vyfoceno, ohniskova vzdalenost, jak se jmenuje fotak atd. . priklad dokument - kdo dokument napsal, kdy byl vytvoren atd.
	systemove logy - jak zarizeni funguje aby bylo monze diagnostikovat problem, kdo se kdy prihlasil, 
	docasne soubory - typicky se jmenuji .tmp pomocny prostor, ukladani mezivysledku
	rezidualni data - vypis pameti - nealokovana pamet, casti souboru, smazane neprepsane soubory, casti souboru, kusy pameni
	
na zjisteni vyznamu dat je potreba znat jejich format. 

analyza dat - forenzni analyza se zabyva zjistenim dukaznich materialu v oblasti IT, jejich interpretaci a prezentaci. 

pozadavky: zjistit pozadovane informace, nezpochybnitelnost zaveru, oprakovatelny postup, neutralni pohled
